...
export function colourToHex(colour: RGBColour): string {
  let alpha = Math.round(colour.alpha * 255)
    .toString(16)
    .padStart(2, '0');
  let red = colour.red.toString(16).padStart(2, '0');
  let green = colour.green.toString(16).padStart(2, '0');
  let blue = colour.blue.toString(16).padStart(2, '0');
...
export function isLightColour(colour: RGBColour): boolean {
  var yiq = (colour.red * 299 + colour.green * 587 + colour.blue * 114) / 1000;
  return yiq >= 128;
}
