...
import { getFirstEventNormal, getPreviousEventNormal } from '../rundown-utils/rundownUtils.js';
...
export function getCueCandidate(entries: RundownEntries, order: EntryId[], insertAfterId: EntryId | null): string {
  ...
  if (insertAfterId === null) return addAtTop();
  ...
  if (!isOntimeEvent(previousEvent)) {
    ...
    if (!isOntimeEvent(previousEvent)) return addAtTop();
  }
  ...
  function addAtTop() {
    ...
    if (firstEventCue === '1') {
      return '0.1'; //TODO: this could be better
    }
    return '1';
  }
}
