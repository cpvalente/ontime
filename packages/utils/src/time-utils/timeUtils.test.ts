...
/*Guideing pricipels:
 *Linked:   expected start = planed start + delay + offset
 *Unlinked: expected start = planed start + delay (if possible otherwise pushed by the offset)
 */
    ...
    test('behind consumes 1 gap partially', () => {
      ...
      expect(calculateExpectedStart(rundownCached, offset, clock, selectedEventIndex)).toEqual({
        ...
        event3: {
          ...
          expectedEnd: 6 * MILLIS_PER_SECOND,
          timeUntil: 2.5 * MILLIS_PER_SECOND,
        },
        event4: {
          expectedStart: 6 * MILLIS_PER_SECOND,
          expectedEnd: 7 * MILLIS_PER_SECOND,
          timeUntil: 3.5 * MILLIS_PER_SECOND,
        },
      });
    });
    ...
    test('behind consumes 2 gaps partially', () => {
      ...
      expect(calculateExpectedStart(rundownCached, offset, clock, selectedEventIndex)).toEqual({
        ...
        event2: {
          ...
          expectedEnd: 4.75 * MILLIS_PER_SECOND,
          timeUntil: 0 * MILLIS_PER_SECOND,
        },
        event3: {
          expectedStart: 5 * MILLIS_PER_SECOND,
