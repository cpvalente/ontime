...
test('Rearrange while playing', async ({ page }) => {
  ...
  await expect(page.getByTestId('entry-2').locator('#event-block')).toMatchAriaSnapshot(`
    - button:
      - img
    - text: "2"
    - textbox "Start" [disabled]: /\\d+:\\d+:\\d+/
    - text: S
    - img
    - textbox "End" [disabled]: /\\d+:\\d+:\\d+/
    - text: E
    - img
    - textbox "Duration": /\\d+:\\d+:\\d+/
    - text: D
    - img
    - textbox "Event title"
    - button "Skip event" [disabled]
    - button "Load event"
    - button "Pause event"
    - img
    - img
  ...
  // but entry 1 should be the one playing
  await expect(page.getByTestId('entry-1').locator('#event-block')).toMatchAriaSnapshot(`
    - button:
      - img
    - text: "2"
    - textbox "Start" [disabled]: /\\d+:\\d+:\\d+/
