...
test('sheet file upload', async ({ page }) => {
  ...
  const worksheetOption1 = await page.locator('[id="event\\ schedule"]').selectOption('Sheet2');
  expect(worksheetOption1).toEqual(['Sheet2']);

  const worksheetOption2 = await page.locator('[id="event\\ schedule"]').selectOption('test');
  expect(worksheetOption2).toEqual(['test']);
