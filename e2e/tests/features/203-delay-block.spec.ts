...
test('delay blocks add time to events', async ({ page }) => {
  ...
  await page.getByText('New start: 00:12:00').click();

  // make negative delay
  await page.getByText('Subtract time').click();
  await page.getByText('New start: 00:08:00').click();
  ...
  await page.getByTestId('delay-input').press('Enter');
  await page.getByText('New start: 00:18:00').click();

  // cancel delay
  await page.getByRole('button', { name: 'Cancel' }).click();
  await expect(page.getByTestId('rundown').getByTestId('time-input-timeStart')).toHaveValue('00:08:00');
  await expect(page.getByText('New start: 00:18:00')).toHaveCount(0);
});
...
test('delays are show correctly', async ({ page }) => {
  ...
  await page.getByText('New start: 00:11:00').click();
