...
export function deleteBlueprint(id: string): void {
  ...
  for (let i = 0; i < automations.length; i++) {
    ...
    if (automation.blueprintId === id) {
      throw new Error(`Unable to delete blueprint used in ${automation.title}`);
    }
  }
 ...
 */
function saveChanges(patch: Partial<AutomationSettings>) {
  const automation = getDataProvider().getAutomation();
  getDataProvider().setAutomation({ ...automation, ...patch });
}
