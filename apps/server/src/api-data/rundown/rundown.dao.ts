...
export function createTransaction(mutableRundown: boolean, mutableCustomFields: boolean): Transaction {
  const rundown = mutableRundown ? structuredClone(cachedRundown) : cachedRundown;
  const customFields = mutableCustomFields ? structuredClone(projectCustomFields) : projectCustomFields;
