...
import { getProjectCustomFields } from './rundown.dao.js';
import { createCustomField, editCustomField, deleteCustomField } from './rundown.service.js';
...
router.delete('/:key', validateDeleteCustomField, async (req: Request, res: Response<CustomFields | ErrorResponse>) => {
  try {
    ...
    res.status(204).send(customFields);
  } catch (error) {
    const message = getErrorMessage(error);
    res.status(400).send({ message });
  }
