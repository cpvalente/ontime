...
export function safeMerge(existing: DatabaseModel, newData: Partial<DatabaseModel>) {
  const { rundown, project, settings, viewSettings, osc, aliases, userFields, previouslyUploaded } = newData || {};
  return {
    ...existing,
    ...
    previouslyUploaded: previouslyUploaded ?? existing.previouslyUploaded,
    userFields: {
      ...existing.userFields,
