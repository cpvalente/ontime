import { OntimeEvent, Playback, ReportData } from 'ontime-types';
...
describe('reporter', () => {
  ...
  test('event start', async () => {
    mockState.eventNow = { id: 'test' } as OntimeEvent;
  ...
  test('event stop', async () => {
    mockState.eventNow = { id: 'test', duration: 500 } as OntimeEvent;
    mockState.timer.startedAt = 666;
    eventStart(mockState);
    expect(generate()).toMatchObject({ test: { ...blankReportData, startAt: 666 } });
