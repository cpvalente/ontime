...
describe('load()', () => {
  ...
  it('dose not load missing newline', () => {
    ...
    const expected = null;
    expect(testLoad).toStrictEqual(expected);
    expect(loggereMock).toHaveBeenLastCalledWith(
      LogOrigin.Server,
      'Invalid restore state: Error: Missing newline character in restore file',
    );
    fs.rmSync(testFolder, { recursive: true, force: true });
  });
  ...
  it('dose not load wrong play state', () => {
    ...
    expect(loggereMock).toHaveBeenLastCalledWith(
      LogOrigin.Server,
      'Invalid restore state: Error: Could not phrase element to Playback state: Play',
    );
    fs.rmSync(testFolder, { recursive: true, force: true });
  });
  ...
  it('dose not load non existing ID', () => {
    ...
    expect(loggereMock).toHaveBeenLastCalledWith(
      LogOrigin.Server,
      'Invalid restore state: Error: Event ID dose not exits: abcd',
    );
    fs.rmSync(testFolder, { recursive: true, force: true });
  });
  ...
  it('dose not load wrong numbers', () => {
    ...
    expect(loggereMock).toHaveBeenLastCalledWith(
      LogOrigin.Server,
      'Invalid restore state: Error: Could not phrase element to number: bad',
    );
    fs.rmSync(testFolder, { recursive: true, force: true });
  });
  ...
  it('dose not load missing file', () => {
    ...
    expect(loggereMock).toHaveBeenLastCalledWith(
      LogOrigin.Server,
      expect.stringContaining('Invalid restore state: Error: ENOENT: no such file or directory, open'),
    );
  });
});
