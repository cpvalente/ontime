...
export async function getUnderstudy(): Promise<MaybeString> {
  ...
  await config.read();
  return config.data.understudy ?? null; // default to null
}

export async function setUnderstudy(understudy: MaybeString): Promise<MaybeString> {
  config.data.understudy = understudy;
  await config.write();
  return understudy;
}
