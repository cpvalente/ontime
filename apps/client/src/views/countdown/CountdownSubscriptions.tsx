  ...
  return (
    <div className='list-container'>
      {subscribedEvents.map((event) => {
        ...
        const { message, timer } = getSubscriptionDisplayData(time, event, selectedId, 0, currentDay, showProjected);
        const display = getFormattedTimer(timer, TimerType.CountDown, getLocalizedString('common.minutes'), {
          removeSeconds: true,
        ...
        return (
          <div key={event.id} className={cx(['sub', isLive && 'sub--live'])}>
            ...
            <div className='sub__playback'>{message}</div>
            ...
            <div className='sub__timer'>{display}</div>
          </div>
        );
      })}
