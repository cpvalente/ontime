 ...
 */
export function getLargeClockData(clock: number, timeformat: string | null, timezoneDelta: number) {
  const [display, meridian] = (() => {
    const formatted = formatTime(clock, { override: timeformat, timezoneDelta });
