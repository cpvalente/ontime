...
import { PanelBaseProps, settingPanels, SettingsOption } from '../settingsStore';
  ...
  return (
    <ul className={style.tabs}>
      {settingPanels.map((panel) => {
        ...
        if (panel.attention) {
          return (
            <Tooltip key={panel.id} label={panel.attention} openDelay={tooltipDelayFast} shouldWrapChildren>
...
function PanelListItem(props: PanelListItemProps) {
  ...
  const classes = cx([
    ...
    panel.attention && style.attention,
  ]);
